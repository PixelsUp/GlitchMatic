...
public class _FinalCam : MonoBehaviour
{
    public static _FinalCam Instance { get; private set; }
    ...
    {
        // Find the player object in the new scene (assuming it's tagged as "Character")
        player = GameObject.FindWithTag("Character(Clone)")?.transform;

        if (player == null)
        {
            Debug.LogError("Player not found in the scene! Make sure the player is tagged as 'Character'.");
        }
    }
    ...
    void LateUpdate()
    {
        ...
        Debug.Log("Camera moving to: " + smoothedPosition);
