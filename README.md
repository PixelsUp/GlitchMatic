# PixelsUp

# GDD

## PixelsUp - GlitchMatic  
**September 25th, 2024**

### Designed by PixelsUp:  
Iván Jiménez Blas, Sergio López Llorente, Sonia Montero Nogales, Álvaro Redondo Molina, Alejandro Serrejón Beltrán, Marcos Vivar Muiño

---

## Index

1. [Introduction](#introduction)  
    1.1 [Brief Concept Description](#brief-concept-description)  
    1.2 [Brief Story and Characters Description](#brief-story-and-characters-description)  
    1.3 [Purpose, Target Audience, and Platforms](#purpose-target-audience-and-platforms)  
2. [Monetization and Business Model](#monetization-and-business-model)  
    2.1 [Monetization Model](#monetization-model)  
    2.2 [Product and Pricing Tables](#product-and-pricing-tables)  
3. [Planning and Costs](#planning-and-costs)  
    3.1 [Human Team](#human-team)  
    3.2 [Estimated Development Time](#estimated-development-time)  
    3.3 [Associated Costs](#associated-costs)  
4. [Gameplay Mechanics and Game Elements](#gameplay-mechanics-and-game-elements)  
    4.1 [Detailed Concept](#detailed-concept)  
    4.2 [Detailed Mechanics](#detailed-mechanics)  
    4.3 [Controls](#controls)  
    4.4 [Levels and Missions](#levels-and-missions)  
    4.5 [Objects, Weapons, and Power-ups](#objects-weapons-and-power-ups)  
    4.6 [Software Architecture](#software-architecture)  
5. [Backstory](#backstory)  
    5.1 [Detailed Story](#detailed-story)  
    5.2 [Characters](#characters)  
    5.3 [Environments and Places](#environments-and-places)  
6. [Art](#art)  
    6.1 [General Aesthetic](#general-aesthetic)  
    6.2 [Pixel Art](#pixel-art)  
    6.3 [Visual Style](#visual-style)  
    6.4 [Concept Art](#concept-art)  
    6.5 [2D Pixel Art](#2d-pixel-art)  
7. [Animation](#animation)  
    7.1 [Animation Style](#animation-style)  
    7.2 [Character Animations](#character-animations)  
    7.3 [Enemy Animations](#enemy-animations)  
    7.4 [Rigging & Skinning](#rigging-skinning)  
    7.5 [Scenario Animations](#scenario-animations)  
8. [Sound](#sound)  
    8.1 [Ambient Sound and Music](#ambient-sound-and-music)  
    8.2 [Sound Effects](#sound-effects)  
    8.3 [Sound List](#sound-list)  
9. [Interface](#interface)  
    9.1 [Basic Menu Designs](#basic-menu-designs)  
    9.2 [Flow Diagram](#flow-diagram)  
10. [Development Roadmap](#development-roadmap)  
    10.1 [Milestone 1](#milestone-1)  
    10.2 [Milestone 2](#milestone-2)  
    10.3 [Milestone 3: Testing & Polishing](#milestone-3-testing-polishing)

---

## 1. Introduction

In **GlitchMatic**, players get a 2D pixel-art, roguelike game where they take control of a character trapped in a tower of glitches. The tower is riddled with broken environments, malfunctioning enemies, and an unstable reality. The character believes the solution to fix the world’s glitches lies at the top of the tower, but their own personal glitch causes them to respawn at the entrance every time they die, locking them in a loop of infinite attempts to climb to the top.

### 1.1 Brief Concept Description

The game combines elements of speedrunning and roguelike gameplay. The player races against time through glitchy environments, defeating enemies and solving puzzles to progress. Death resets them to the beginning, with each respawn offering a slightly altered experience. The game emphasizes replayability, fast reflexes, and strategic thinking.

### 1.2 Brief Story and Characters Description

- **Main Character**: A "Glitch Hunter" who enters the tower with a mission to fix the glitches that have corrupted the world. However, he is unaware of his own glitch: every time he dies, he is reset to the tower's entrance. The game’s loop revolves around this endless pursuit, with no one ever truly reaching the top.
- **Tower Entities**: Various glitch-based enemies that inhabit the tower, each with unique behaviors affected by the world’s instability.

### 1.3 Purpose, Target Audience, and Platforms

- **Purpose**: Provide an immersive roguelike experience with a unique glitch-based narrative, challenging speedrunners and casual players alike.
- **Target Audience**: Fans of roguelikes, speedrunners, and those who enjoy narrative-driven, pixel-art games like *The Binding of Isaac* and *Celeste*. The game focuses on both competitive players seeking a fast-paced experience and casual players enjoying its thematic design.
- **Platforms**: PC, potential expansion to consoles.

---

## 2. Monetization and Business Model

### Monetization Model

- The game will be sold as a premium title with potential for future DLCs or expansions.
- In-game purchases will be limited to cosmetic items to ensure fair play for all users.

### Product and Pricing Tables

- **Base Game**: Full game available for purchase at an affordable price point.
- **DLCs/Expansions**: Potential post-launch expansions.
- **Cosmetic Upgrades**: Optional purchases for character customization.

---

## 3. Planning and Costs

### Human Team

- Six members: Game design, programming, art, sound, and project management roles.

### Estimated Development Time

- Four months of development time with clear milestones and checkpoints.

### Associated Costs

- **Software licenses**: Game engine, art software, and sound editing tools.
- **Marketing and promotion budget**: Social media campaigns, trailers, and early access demos.
- **Potential porting to consoles**: Costs for porting the game from PC to consoles.

---

## 4. Gameplay Mechanics and Game Elements

### Detailed Concept

- **Core Gameplay Loop**: Players progress through rooms filled with enemies and glitches, aiming to clear each level as quickly as possible. Upon death, the game restarts, with slight variations in the tower layout and AI behavior.

### Detailed Mechanics

- **Movement**: Standard movement mechanics (run, attack).
- **Combat**: Fast-paced, with a variety of weapons (glitch guns, melee attacks).
- **Time Mechanic**: Players race to clear as many rooms as possible before dying.
- **Upgrading**: As players progress through the levels, they can upgrade their character, making each playthrough easier.

### Controls

- Simple and intuitive controls designed for fast gameplay and fluid combat.

### Levels and Missions

- **Rooms**: Procedurally generated rooms with increasing difficulty.
- **Glitch Hazards**: Obstacles that alter gameplay unpredictably.

### Objects, Weapons, and Power-ups

- **Glitch Weapons**: Reflect the chaotic nature of the world.
- **Power-ups**: Personalize each playthrough and empower the player.

### Software Architecture

- **Modular structure**: AI, room generation, and combat systems are modular, allowing dynamic development and testing.

---

## 5. Backstory

### Detailed Story

- The tower represents the central point of time-world corruption. The **Glitch Hunter** enters the tower, hoping to fix the glitches and restore the timeline. However, the Glitch Hunter is trapped in an endless loop, cursed to reset upon death and never truly reach the top. The story unfolds gradually as the player ascends the tower.

### Characters

- **The Glitch Hunter**: A lone protagonist determined to fix the glitches.
- **Tower Inhabitants**: Various glitchy enemies whose behavior evolves based on player actions. These entities came from different timelines and now seek to stop the Glitch Hunter to maintain their newfound freedom.

### Environments and Places

- **Tower Rooms**: Procedurally generated rooms with varied challenges that enhance replayability.

---

## 6. Art

### General Aesthetic

- **Pixel Art**: Retro-inspired with glitch effects that emphasize the game's chaotic world.
- **Visual Style**: Dark and futuristic, with an emphasis on digital corruption.

### Concept Art

- Early sketches of characters, weapons, and enemies guide the final visual assets.

### 2D Pixel Art

- All game assets are rendered in detailed, retro-style pixel art.

---

## 7. Animation

### Animation Style

- Smooth, responsive animations for both player characters and enemies.

### Character Animations

- Fluid animations for running, jumping, and attacking.

### Enemy Animations

- Erratic, glitchy movements that reflect the unstable environment.

### Rigging & Skinning

- Detailed character rigs for fluid motion in all actions.

### Scenario Animations

- Glitchy environmental elements that change unpredictably.

---

## 8. Sound

### Ambient Sound and Music

- A glitch-inspired soundtrack filled with synthetic and distorted sounds to reflect the unstable world.

### Sound Effects

- Fitting effects for weapon use, enemy interactions, and environmental changes.

### Sound List

- Footsteps, weapon attacks, enemy and character growls, and environment glitches.

---

## 9. Interface

### Basic Menu Designs

- Glitch-themed UI to reflect the game's digital instability.

### Flow Diagram

- A clean, logical flow from menu to gameplay.

---

## 10. Development Roadmap

### Milestone 1

- Finalize the core game concept.
- Develop early concept art and design the gameplay loop.

### Milestone 2

- Implement core mechanics (movement, combat, AI behavior).
- Begin creating the procedural room generation system.

### Milestone 3: Testing & Polishing

- Playtesting and balancing.
- Finalizing assets, animations, and sound design.

### Release Date

- **Estimated Release Date**: To be determined after the final polish phase.
